...
# from MXF  
  ...
  SMPTE_INTERMEDIATE_FILE_SUFFIX = 'tiff'
  MXFI_INTERMEDIATE_FILE_SUFFIX = 'jpg'
  ...
  def CinemaslidesCommon::split_indices(source)
    ...
    if (n_elements == 0) 
      remainder.times { |i| indices << [i,i] }
    else
      start_index = 0; end_index = n_elements - 1
      (n_threads-remainder).times do |i|
	indices << [start_index, end_index]
	start_index += n_elements; end_index += n_elements
      end
      end_index += 1
      remainder.times do |i|
	indices << [start_index, end_index]
	start_index += n_elements + 1 ; end_index += n_elements + 1
      end
    end
    return indices
  end
